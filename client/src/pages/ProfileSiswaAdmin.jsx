    ...
    MySwal.fire({
      ...
      html: `
        <div id="formEdit" class="form-container">
          ...
          <input id="nama" type="text" class="swal2-input" value="${siswa.nama || ''}" />
          <div style="display: flex; gap: 10px;">
            <div style="flex: 1;">
              <label for="nis">NIS</label>
              <input id="nis" type="text" class="swal2-input" value="${siswa.nis || ''}" />
            </div>
            <div style="flex: 1;">
              <label for="nisn">NISN</label>
              <input id="nisn" type="text" class="swal2-input" value="${siswa.nisn || ''}" />
            </div>
          </div>
          <label for="tgl">TANGGAL LAHIR</label>
          <input id="tgl" type="date" class="swal2-input" value="${formatTanggal(siswa.tanggal_lahir) || ''}" />
      ...
      if (result.isConfirmed) {
        try {
          const token = getToken();
          await axios.put(`http://localhost:8000/api/siswas/${id}`, result.value, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
      ...
      <div className="profile-page">
        ...
        <div className="content-container">
          ...
          <div className="nis-nisn">
            ...
            <input className="tanggal-lahir-value" type="date" value={siswa.tanggal_lahir || ''} readOnly />
            <label htmlFor="" className="alamat">ALAMAT</label>
            <input type="text" className="alamat-value" value={siswa.alamat || ''} readOnly />
          </div>
          <button className="edit-data-btn" onClick={handleEditData}><BiSolidPencil /></button>
        </div>
      </div>
    </>
  );
